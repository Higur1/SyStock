...
export default class UserService {
  ...
  static async findByEmail(userData: IUser) {
    try {
      const findResult = await UserModel.findByEmail(userData);
  ...
  static async create(userData: IUser) {
    try {
      ...
      const verifyPreUser = await PreUserModel.find(preUserData);
  ...
  static async resetPassword(token, password) {
    try {
      ...
      const resultUpdatedPassword =
        await UserModel.updatePassword_resetPassword(
          validToken.tokenIsValid?.user_id,
          validToken.tokenIsValid?.token,
          hashPassword
        );
